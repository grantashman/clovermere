...
func _initialize() -> void:
    ...
    actor.set_route([])
    require(actor.idle_variant() == "sorting", "gathering residents should show a sorting idle cue")
    actor.set_activity("tending")
    require(actor.idle_variant() == "watering", "garden residents should show a watering idle cue")
    actor.set_activity("crafting")
    require(actor.idle_variant() == "hammering", "makers should show a hammering idle cue")
    actor.set_activity("resting")
    require(actor.idle_variant() == "resting", "resting residents should show a resting idle cue")
