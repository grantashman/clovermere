...
func _initialize() -> void:
    ...
    for asset_id in ["cottage", "workshop", "hall", "garden", "barn", "tree", "stone", "herb", "player", "resident"]:
    ...
    for facade_id in ["hall", "garden", "barn"]:
        require(ArtAssetPack.facade_asset_for({"hall": "clovermere-hall", "garden": "herbalists-garden", "barn": "old-barn"}[facade_id]) == facade_id, "%s should resolve from its building id" % facade_id)
